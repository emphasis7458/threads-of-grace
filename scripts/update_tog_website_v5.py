#!/usr/bin/env python3
"""
Threads of Grace Website Update Script v5

Fixes:
1. Improved title extraction: checks <h1> first, falls back to first <p>
2. Filters out scripture readings AND liturgical occasions from being treated as titles
3. Updates index pages: Title → Date + Occasion → Scripture
4. Updates meditation counts in by-season.html

Usage:
    python update_tog_website_v5.py /path/to/threads-of-grace-website --analyze
    python update_tog_website_v5.py /path/to/threads-of-grace-website
"""
...
def looks_like_occasion(text):
    """
    Determine if text looks like a liturgical occasion rather than a title.
    Examples:
        "The First Sunday after Christmas, Year B" -> True
        "Advent 1, Year B" -> True
        "Liturgy of the Palms" -> True
        "Fifth Sunday in Lent" -> True
        "GIFTS" -> False
        "Resurrection Grace" -> False
    """
    if not text:
    ...
    # ALL CAPS text is typically a meditation title, not an occasion name
    # Occasion names appear in Title Case like "First Sunday of Advent, Year B"
    if text.isupper() and len(text) > 10:
        return False
    
    text_lower = text.lower()
    
    # Patterns that indicate occasion text
    occasion_patterns = [
        r'year\s*[abc]',                          # "Year A", "Year B", "Year C"
        r'\b(first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|twenty.first|twenty.second|twenty.third|twenty.fourth|twenty.fifth|twenty.sixth|twenty.seventh|last)\s+sunday',  # "First Sunday", etc.
        r'\bsunday\s+(of|after|in)\s+',           # "Sunday of Easter", "Sunday after Pentecost"
        r'\badvent\s*\d',                         # "Advent 1", "Advent 2"
        r'\blent\s*\d',                           # "Lent 1", "Lent 2"  
        r'\beaster\s*\d',                         # "Easter 1", "Easter 2"
        r'\bproper\s*\d',                         # "Proper 21"
        r'\bliturgy\s+of\s+the\s+(palms|word)',   # "Liturgy of the Palms"
        r'\beaster\s+day',                        # "Easter Day"
        r'\bchristmas\s+day',                     # "Christmas Day"
        r'\bpalm\s+sunday',                       # "Palm Sunday"
        r'\bash\s+wednesday',                     # "Ash Wednesday"
        r'\bgood\s+friday',                       # "Good Friday"
        r'\btrinity\s+sunday',                    # "Trinity Sunday"
        r'\bpentecost',                           # "Day of Pentecost", etc.
        r'\bthe\s+epiphany',                      # "The Epiphany"
        r'\bchrist\s+the\s+king',                 # "Christ the King"
        r'\ball\s+saints',                        # "All Saints"
        r'\bbaptism\s+of\s+(our\s+)?lord',        # "Baptism of Our Lord"
        r'\btransfiguration',                     # "The Transfiguration"
        r'\bholy\s+name',                         # "The Holy Name"
    ]
...
def is_valid_title(text):
    ...
    # BUT allow: ellipses (. . .), abbreviations (vs., Mr., Dr., St., etc.)
    # Check for sentence-ending pattern: word + period + space + Capital (new sentence)
    # Exclude common abbreviations and ellipsis patterns
    temp_text = text
    # Remove ellipsis patterns before checking
    temp_text = re.sub(r'\.\s*\.\s*\.', '...', temp_text)  # normalize ellipsis
    temp_text = re.sub(r'\.\.\.', '', temp_text)  # remove ellipsis
    # Remove common abbreviations
    temp_text = re.sub(r'\b(vs|Mr|Mrs|Ms|Dr|St|Rev|Jr|Sr|etc)\.\s', ' ', temp_text, flags=re.IGNORECASE)
    # Now check for multi-sentence pattern
    ...
    # Liturgical occasions aren't titles
...
def extract_title_from_meditation(filepath):
    ...
    Strategy:
    1. First check <h1 class="meditation-title-display"> - if it's a valid title, use it
    2. If h1 is scripture/occasion, check <p> tags in meditation-content div
       - Skip scripture readings and occasion labels
       - Title is typically ALL CAPS and short (< 100 chars)
    """
    try:
        ...
        # Source 2: Check <p> tags in meditation-content div
        # The title may be in the 2nd, 3rd, or 4th <p> if earlier ones are scripture
        ...
        if content_div:
            ...
            for i, p in enumerate(paragraphs[:5]):  # Check first 5 paragraphs
                ...
                # Skip scripture readings
                if looks_like_scripture(p_text):
                    continue
                
                # Skip occasion labels (like "Liturgy of the Word")
                if looks_like_occasion(p_text):
                    continue
                
                # Valid title candidate - typically ALL CAPS and short
                if is_valid_title(p_text):
                    return p_text, f'p{i+1}'
...
def build_meditation_data(meditations_dir):
    ...
    for html_file in sorted(Path(meditations_dir).glob('*.html')):
        ...
        if title:
            # Normalize source for counting (p1, p2, p3 -> p)
            source_key = 'p' if source and source.startswith('p') else source
            title_sources[source_key] += 1
        else:
            title_sources['none'] += 1
            missing_titles.append(date_str)
    
    print(f"\n  Title extraction results:")
    print(f"    From <h1> (properly formatted): {title_sources.get('h1', 0)}")
    print(f"    From <p> (title in content):    {title_sources.get('p', 0)}")
    print(f"    No title found:                 {title_sources.get('none', 0)}")
...
def main():
    ...
    p_samples = [(d, m) for d, m in sorted(meditation_data.items(), reverse=True) 
                 if m.get('title') and m.get('title_source', '').startswith('p')]
    ...
    for date, data in p_samples[:3]:
        title = title_case_smart(data['title'])
        source = data.get('title_source', 'p')
